...
export function DslPlayground() {
  ...
  // Track typing state: true while user is actively editing, clears 1.5s after last keystroke
  ...
  return (
    <div className="min-h-full bg-zinc-50 dark:bg-zinc-950 text-zinc-800 dark:text-zinc-100 flex flex-col p-6 gap-6 transition-colors duration-300">
      ...
      <header className="shrink-0 relative flex flex-col sm:flex-row justify-between sm:items-center gap-4 border-b border-zinc-200 dark:border-zinc-800 pb-5 overflow-hidden">
        ...
        <div className="flex items-center gap-3 shrink-0 relative z-10">
          <Button 
            ...
            isLoading={isRunning} 
            className="cursor-pointer relative overflow-hidden group border border-zinc-900/10 dark:border-cyan-accent/25 hover:border-zinc-900/20 dark:hover:border-cyan-accent text-white dark:text-zinc-950 bg-zinc-900 hover:bg-zinc-800 dark:bg-cyan-accent dark:hover:bg-cyan-accent/90 px-5 py-2.5 rounded-lg font-semibold tracking-wide shadow-sm dark:shadow-[0_0_15px_var(--color-cyan-glow)] dark:hover:shadow-[0_0_25px_var(--color-cyan-glow)] active:scale-[0.98] transition-all duration-200"
          >
      ...
      <div className="flex-grow grid grid-cols-1 lg:grid-cols-12 gap-6 min-h-0">
        ...
        <div className="lg:col-span-7 flex flex-col min-h-[500px] lg:min-h-0 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden shadow-sm">
          {/* Header */}
          ...
          <div className="flex-1 w-full bg-zinc-900 dark:bg-zinc-950">
            <Editor
              height="100%"
        ...
        <div className="lg:col-span-5 flex flex-col min-h-[400px] lg:min-h-0 gap-4">
          ...
          {/* Active Tab Panel */}
          <div className="flex-grow bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden flex flex-col shadow-sm min-h-0">
            ...
            {/* Trace Tab */}
            {activeTab === 'trace' && (
              <div className="flex-1 flex flex-col h-full min-h-0 bg-white dark:bg-zinc-900">
                <div className="bg-zinc-100 dark:bg-zinc-950/60 border-b border-zinc-200 dark:border-zinc-900 px-4 py-3 text-xs font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-widest flex items-center justify-between shrink-0">
                  <span>Sandbox Trace Logs</span>
                  <span className="w-2 h-2 rounded-full bg-cyan-accent shadow-[0_0_8px_#00f0ff]"></span>
                </div>
                <div className="p-4 flex-grow overflow-y-auto font-mono text-xs min-h-0 bg-zinc-50/40 dark:bg-zinc-950/20">
                  {runTrace.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center text-center p-6 min-h-[300px]">
                      <div className="w-12 h-12 rounded-full bg-zinc-100 dark:bg-zinc-950 text-zinc-400 dark:text-zinc-500 flex items-center justify-center mb-3 border border-zinc-250 dark:border-zinc-800/80">
                        <Terminal className="w-6 h-6 text-cyan-600 dark:text-cyan-accent animate-pulse" weight="bold" />
                      </div>
                      <h3 className="text-xs font-bold text-zinc-700 dark:text-zinc-300 uppercase tracking-wider">No Sandbox Execution</h3>
                    ...
                    <div className="flex flex-col gap-2.5">
                      {runTrace.map((trace, i) => (
                        <div key={i} className="flex gap-4 text-zinc-650 dark:text-zinc-350 border-l border-zinc-200 dark:border-zinc-850 pl-4 py-2 relative group hover:bg-zinc-100/30 dark:hover:bg-zinc-900/30">
                          {/* Dot indicator on vertical line */}
                          <span className={`absolute -left-[4.5px] top-[14px] w-2 h-2 rounded-full ${
                            trace.status === 'ERROR' ? 'bg-red-500' : trace.status === 'COMPLETE' ? 'bg-emerald-500' : 'bg-zinc-400 dark:bg-zinc-650'
                          }`}></span>
                          
                          <span className="text-zinc-400 dark:text-zinc-550 font-semibold select-none shrink-0 text-[10px] pt-[2.5px] font-mono">
                            {new Date(trace.timestamp).toISOString().split('T')[1].replace('Z', '')}
                          </span>
                          <span className={`flex-1 text-[12.5px] leading-relaxed font-mono ${
                            trace.status === 'ERROR' ? 'text-red-650 dark:text-red-400 font-medium' : trace.status === 'COMPLETE' ? 'text-emerald-650 dark:text-emerald-450 font-semibold' : 'text-zinc-750 dark:text-zinc-300'
                          }`}>
                            {trace.description}
                          </span>
                          <span className={`shrink-0 font-bold text-[9px] uppercase tracking-widest px-2 py-0.5 rounded border self-center font-mono ${
                            trace.status === 'OK' 
                              ? 'text-cyan-700 dark:text-cyan-accent border-cyan-200 dark:border-cyan-accent/20 bg-cyan-50 dark:bg-cyan-accent/5' 
                              : trace.status === 'ERROR' 
                              ? 'text-red-650 dark:text-red-455 border-red-200 dark:border-red-500/20 bg-red-50 dark:bg-red-500/5' 
                              : 'text-emerald-650 dark:text-emerald-455 border-emerald-250 dark:border-emerald-400/25 bg-emerald-50 dark:bg-emerald-400/5'
                          }`}>
                            {trace.status}
                          </span>
                        </div>
